                    ...
                    <div class="card mx-auto custom-card mb-5" id="prova">
                        ...
                        <div class="card-header d-flex justify-content-between align-items-center bg-white pl-3 pr-1 py-2">
                            ...
                            <div class="card-dots">
                                ...
                                <button type="button" class="btn btn-link text-muted" data-bs-toggle="modal" data-bs-target="#post{{$post->id}}">
                                ...
                                <div class="modal fade" id="post{{$post->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                    ...
                                    <div class="modal-content">
                                        <ul class="list-group">
                                            ...
                                            <a href="#"><li class="btn list-group-item" data-bs-dismiss="modal">Cancel</li></a>
                                        </ul>
                                    </div>
                                    </div>
                                </div>
                        ...
                        <div class="card-body px-3 py-2">
                                ...
                                <form method="POST" action="{{  route("like.create",['like'=>$post->id])}}">
                                    ...
                                    @if($post->like->isEmpty())
                                        <button type="submit" class="btn pl-0">
                                            <i class="far fa-heart fa-2x" data-id="{{ $post->id }}"></i>
                                        </button>
                                    @else
                                        ...
                                        @if($state)
                                            <button type="button " class="btn pl-0 btn_love" data-id="{{ $post->id }}" data-url="{{  route("like.create",['like'=>$post->id])}}">
                                    ...
                                    <div class="modal fade" id="sharebtn{{$post->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                        ...
                                        <div class="modal-content">
                                            <ul class="list-group">
                                                ...
                                                <li class="btn list-group-item" data-bs-dismiss="modal" onclick="copyToClipboard('copy_{{ $post->id }}')">Copy Link</li>
                                                <li class="btn list-group-item" data-bs-dismiss="modal">Cancel</li>
                                            </ul>
                                        </div>
                                        </div>
                                    </div>
                            ...
                            <div class="flex-row">
                                ...
                                @if (count($post->like->where('State',true)) > 0)
                                    <h6 class="card-title">
                                        <strong> <span id="count{{$post->id}}">{{ count($post->like->where('State',true)) }}</span> likes</strong>
                                    </h6>
                                @endif
            ...
            <aside class="aside col-md-4 py-3">
                <div class="position-fixed">
                    ...
                    <div class="d-flex justify-content-start bd-highlight mb-3">
                        <a href="/profile/{{Auth::user()->username}}" style="width: 80px; height: 80px;">
                    ...
                    <div class='mb-4' style="width: 300px">
                        <h6 class='text-secondary'>Rekomendasi untuk kamu</h5>
                        ...
                        @foreach ($sugg_users as $sugg_user)
                            ...
                            <div class='suggestions py-2'>
                                 <div class="d-flex justify-content-start bd-highlight mb-3">
                                        <a href="/profile/{{$sugg_user->username}}" style="width: 64px; height: 64px;">
                                            <img src="{{ asset($sugg_user->profile->getProfileImage()) }}" class="rounded-circle w-50">
                                        </a>
                                    <div class='d-flex flex-column pl-3'>
                                        <a href="/profile/{{$sugg_user->username}}" class='h6 m-1 text-dark text-decoration-none' >
                                            <strong>{{ $sugg_user->name}}</strong>
                                        </a>
                                        <small class="text-muted">Baru di instagram </small>
                                    </div>
